...
const WHATSAPP_NUMBER = '5519998094210'
...
export default function ObrigadoContent() {
  ...
  function buildMsg() {
    if (!nome) return 'Olá! Acabei de preencher o formulário.'
    const c = categoria ? categoria.charAt(0).toLowerCase() + categoria.slice(1).replace(/\.$/, '') : ''
    const r = resposta  ? resposta.charAt(0).toLowerCase()  + resposta.slice(1).replace(/\.$/, '')  : ''
    if (!c && !r) return `Olá, me chamo ${nome}.`
    if (!r) return `Olá, me chamo ${nome}, ${c}.`
    return `Olá, me chamo ${nome}, ${c} e ${r}.`
  }
